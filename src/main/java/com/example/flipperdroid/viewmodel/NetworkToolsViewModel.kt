...
class NetworkToolsViewModel : ViewModel() {
    ...
    fun copyNmapToTmp(context: Context): String {
        ...
        try {
            ...
            // Copier récursivement tout le contenu depuis assets/nmap/
            Log.d("NMAP_COPY", "Copie récursive depuis assets/nmap/ vers ${internalNmapDir.absolutePath}")
            copyAssetsRecursively(context, "nmap", internalNmapDir)
            
            // Copier vers /data/local/tmp avec root
            ...
            // Créer les fichiers de configuration manquants pour éviter les warnings Termux
    ...
    private fun copyAssetsRecursively(context: Context, assetPath: String, destDir: File) {
        try {
            ...
            if (assets == null || assets.isEmpty()) {
                ...
                // C'est un dossier, on le crée et on copie son contenu
    ...
    fun nmap(host: String, parameters: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val defaultParams = if (parameters.isEmpty()) "-sn --max-retries 1 --disable-arp-ping" else parameters
